	...
	/*=========program IO params==========*/
	#define GASCAT_DIR  "/gpfs/data/jvbq85/HBT/data/AqA5/gascat"			//the output directory for gashalocat,gassubcat and gassrccat, this must be an existing directory
	#define SUBCAT_DIR  "/gpfs/data/jvbq85/HBT/data/AqA5/subcatMassPrec"			//the output directory for subcatalogues and srccatalogues, this must be an existing directory
	//necessary sub-dirs: splitters,pro2dest,history
	#define GRPCAT_DIR  "/gpfs/data/aquarius/halo_data/Aq-A/5"				//the input directory for GrpCatalogues
	#define  SNAPSHOT_DIR  "/gpfs/data/aquarius/halo_data/Aq-A/5"					//the input directory for simulation snapshots
	#define LOGFILE_NAME  "logfile"																			//the name of program logfile, set to "stdout" to use stdout

	/*======simulation params===========*/
	#define NP_SIM	2951686LL //total Number of all kinds of particles
	#define NP_GAS 	0
	#define NP_DM 	2316893LL
	#define BOXSIZE 100.0  //Mpc/h
	#define MP_DM   0.000229431
	#define MP_GAS  0.

	/*=======Tree algorithm params========*/
	#define TREE_ALLOC_FACTOR 2//  MaxNodes = ((maxnodes>500)?maxnodes:500);
	#define ErrTolTheta 0.45
	#define SofteningHalo 5e-4  //bug fix: from 0.0006849 to 0.0006849*h
	#define NodeResolution 5e-5  //0.1*Softening
	#define NodeReso_half 2.5e-5
	...
	#define PrecMass 0.9 //relative converge error for unbind (ErrorTolMass=1-PrecMass)
...
//	#define CONVERT_LENGTH_MPC_KPC //this tells to convert unit from Mpc to kpc, 
	...
	#define SNAPFILE_BASE "snap_C02_200"
	#define NFILES 8   //number of files per snapshot
	#define NFILES_GRP 16 //number of group files per snapshot
