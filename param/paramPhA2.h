...
//this is the default A2 subcat (renamed from A2more)
	/*=========program IO params==========*/
	#define SUBCAT_DIR  "/gpfs/data/jvbq85/HBT/data/PhA2/subcat"			//the output directory for subcatalogues and srccatalogues, this must be an existing directory
	//necessary sub-dirs: splitters,pro2dest,history
	#define GRPCAT_DIR  "/gpfs/data/Phoenix/pha-2/"				//the input directory for GrpCatalogues
	#define  SNAPSHOT_DIR  GRPCAT_DIR					//the input directory for simulation snapshots
	#define LOGFILE_NAME  "logfile"																			//the name of program logfile, set to "stdout" to use stdout

	/*======simulation params===========*/
	#define NP_SIM	540598770LL//total Number of all kinds of particles;
	#define NP_GAS 	0
	#define NP_DM  506705980LL//
	#define BOXSIZE 	500.0  //Mpc/h
	#define MP_DM   5.083959e-04
	#define MP_GAS   0.

	/*=======Tree algorithm params========*/
	#define TREE_ALLOC_FACTOR 2//  MaxNodes = ((maxnodes>500)?maxnodes:500);
	#define ErrTolTheta 0.45
	#define SofteningHalo 0.00032  //Mpc/h
	#define NodeResolution (SofteningHalo*0.1)  //0.1*Softening
	#define NodeReso_half (NodeResolution/2.)
