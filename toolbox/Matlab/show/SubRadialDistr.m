...
loglog(r/R1, rho/(M1/mp/R1^3).*(r/R1).^(1.3), 'g-', 'displayname', 'sat Model');
nbin=15;
nmin=100;
...
% rhomean=sum(Rlist1(f)<R1)/R1^3;
rhomean=sum(n1)/R1^3;
h4=errorbar(xr1/R1,rhon1/rhomean,erhon1/rhomean,'-','linewidth',3,'color',[0.8,0.8,1], 'displayname','HBT');
f=Mlist2>nmin*mp;
...
%%
n3=[  310,   511,   850,  1445,  1990,  2938,  4361,  6343,  9447, 11897, 14477,  6396,  2080,     0];
plot(rbin(1:end-1)/R1, n3./diff(rbin.^3)/(sum(n3)/R1^3),'r--');
%%
mbdfile='/work/Projects/SubProf/data/MbdInfall.hdf5';
Mbd.m=h5read(mbdfile,'/mass');
Mbd.x=h5read(mbdfile,'/x')';
Mbd.mTV=h5read(mbdfile,'/massTV')';
Mbd.snapTV=h5read(mbdfile, '/snapTV')';
