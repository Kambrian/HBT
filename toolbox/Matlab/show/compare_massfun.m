...
for i=1:2:nfun
    ...
    plot(log10(xmass)+10,mfun(:,1).*(xmass).^alpha,'-',...
        'color',colors(i), 'linewidth', 3,...
        'displayname',[num2str(log10(data(i).Mbin(1))+10,'%2.1f'),'$\sim$',num2str(log10(data(i).Mbin(2))+10,'%2.1f'),' SatAcc']);
end
hl=legend('show','location','southwest');set(hl,'interpreter','latex');

Nsnap=59; RunNum='8213';
datadir=['/mnt/A4700/data/',RunNum,'/subcatNSM/anal/massfun/'];
[data,redshift]=read_massfun([datadir,'massfun_',num2str(Nsnap,'%03d'),'.',num2str(virtype,'%d')]);%,'.R1.0-2.0']);
nfun=numel(data);
Mhost=zeros(nfun,1);
for i=1:nfun
...
% print('-depsc',[outputdir,'msfunln_8213vs8213NSM_S99V0.eps']);
