//to track the energy evolution of subhalo from snapmax back
#include <stdio.h>
#include <stdlib.h>
#include <math.h>
#include <string.h>
...
{
	SUBCATALOGUE SubCat,SubCatMax;
	int Nsnap,SnapMin=43,SnapMax=63,i,j,pid;
	int SIDMin,SIDMax=6,subid;
	FILE *fp;
	char buf[1024];
...
//prepare particles
	...
	PIndex=SubCatMax.PSubArr[SIDMax];
	N=SubCatMax.SubLen[SIDMax];
	subid=SIDMax;
	pot=mymalloc(sizeof(double)*N);
	Kin=mymalloc(sizeof(double)*N);
	for(Nsnap=SnapMax;Nsnap>=SnapMin;Nsnap--)
	{
		...
		sprintf(buf,"%s/energymap_%03d_%d.%d",outputdir,SnapMax,SIDMax,Nsnap);
		myfopen(fp,buf,"w");
		for(i=0;i<N;i++)
			fprintf(fp,"%g,%g\n",Kin[i],pot[i]);
		fclose(fp);
		sprintf(buf,"%s/posmap_%03d_%d.%d",outputdir,SnapMax,SIDMax,Nsnap);
		myfopen(fp,buf,"w");
		for(i=0;i<N;i++)
		{
			for(j=0;j<3;j++)
