/* equivalent to BT.c except that it has 
 * additional freedom to specify the number 
 * of snapshots to skip at every tracing step*/
#include <stdio.h>
#include <stdlib.h>
#include <math.h>
#include <string.h>
...
/*	if(SnapRange[0]>0)//pick up a previous subcat to continue,assign snapnum first
	{		
			...
			SnapshotNum=SnapRange[0]-SNAP_SKIP;
...
for(SnapshotNum=SnapRange[0];SnapshotNum<=SnapRange[1];SnapshotNum+=SNAP_SKIP)
{
	load_group_catalogue(SnapshotNum,&CatB,fofdir);
	load_particle_data(SnapshotNum,snapdir);	
	#ifndef GRPINPUT_INDEX
	fresh_ID2Index(&CatB,-1); 	//fofcat of JING's data are originally PIndex rather than PID
	#endif
	fresh_ID2Index(&SrcCatB,-3);//now CatB.PIDorIndex has been occupied with Index to the freshly loaded Pdat
