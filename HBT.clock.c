...
clock_t ctime_start,ctime_end,ctward[20]; //for program timing
struct tms tms_start, tms_end;
FILE *fptime;

time_start=time(NULL);
ctime_start=times(&tms_start);
#ifdef HALO_PARA
omp_set_nested(0);
#endif
...
//	fflush(fpsp);
	...
	for(i=1;i<9;i++)
	fprintf(fptime,"%ld ",(ctward[i]-ctward[i-1])/CLOCKS_PER_SEC);
	fprintf(fptime,"\n");fflush(fptime);
}
fclose(fptime);
...
time_end=time(NULL);
ctime_end=times(&tms_end);
fprintf(logfile,"Program %s Finished in %ld minutes (or %ld hours).\n",
             argv[0], (time_end-time_start)/60,(time_end-time_start)/3600);
clock_t utime, stime, alltime;
		...
		alltime=utime+stime
				...
				+(tms_end.tms_cstime-tms_start.tms_cstime); //total processor time (cpu_time*n_cpus)
//fprintf(logfile,"%ld, %ld, %ld\n", CLOCKS_PER_SEC, sysconf(_SC_CLK_TCK), sysconf(_SC_CLK_TCK));
fprintf(logfile,"CPU time %ld, Processor time %ld seconds (%.1f%% user, %.1f%% sys)\n",
		(ctime_end-ctime_start)/sysconf(_SC_CLK_TCK), alltime/sysconf(_SC_CLK_TCK), 
		100.*utime/alltime,
		100.*stime/alltime);
fclose(logfile);
