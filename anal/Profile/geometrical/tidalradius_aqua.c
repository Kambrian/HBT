//use the average background inside some local radius, as used by the Aquarius paper
...
void sub_radius_eq_global(SUBPROFILE *subprof,int SubLen);//consider improving accuracy by interpolation or fitting
...
logfile=stdout;
sprintf(outputdir,"%s/profile/aqua",SUBCAT_DIR);	
...
{	
	...
	subprof[subid].rcen=(SubCat.SubLen[subid]>0)?distance(SUBCOM(subid),SUBCOM(subid)):0;//finding rcen
	sub_radius_eq_global(subprof+subid,SubCat.SubLen[subid]);//calculating req
...
* calculate background inside a radius r with Mtotal(<r)=Msub*/
	...
	int i,nbin,*vol,sum_this,sum_back,sum_other,binmax_sub,nbin_back;
	if(nbin=subprof->nbin)
	{
		...
		if(sum_back+sum_other)
		{
			...
			av_dens_all=(float)(sum_back+sum_other)/(nbin_back*nbin_back*nbin_back);
			for(i=0;i<nbin;i++)
			...
			if(sum_back)
			{
				...
				av_dens_back=(float)sum_back/(nbin_back*nbin_back*nbin_back);
				for(i=0;i<nbin;i++)
