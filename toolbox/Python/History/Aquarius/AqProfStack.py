...
for H in HList:
  Halo[H]=HaloData('Aq'+H+'2')
Halo['a']=HaloData('AqA1')
...
plt.loglog(rSub[denRat>0], denRat[denRat>0], 'gs', label='Aquarius')
...
def plot_haloArat():
  ...
  rHalo,denHalo,denHaloRef,denHaloErr=Halo[H].get_host_density(xbin, Rref)
  rSub,denSub,denSubRef,denSubErr=Halo[H].get_sub_density((Halo[H].m>fMin*Halo[H].Mvir/Halo[H].mP), xbin, Rref)
  plt.plot(rSub[denSub>0], (denSub/denHalo)[denSub>0], 'ro', label=Halo[H].name)
...
for h in HList:
  ...
  f=(n>0)&(x>1e-6)
  pars=powerlaw_fit(x[f],n[f]/np.log(xbin[1]/xbin[0]), w=None)#1/np.sqrt(n)[f])
  NAll.append(n)
  parAll.append(pars[0])
NAll=np.array(NAll)/np.log(xbin[1]/xbin[0])
parAll=np.array(parAll)
print 'M*,alpha:', parAll.mean(0), parAll.std(0)
plt.loglog(x, NAll.mean(0), 's', mfc='none', mec='g', label='Aquarius')
plt.fill_between(x, np.percentile(NAll, 25, axis=0), np.percentile(NAll, 75, axis=0), color='g', alpha=0.3)
plt.yscale('log', nonposy='clip')
#for i in range(7):
  #plt.plot(x, NAll[i])
f=(NAll.sum(0)>0)&(x>1e-6)
pars=powerlaw_fit(x[f],NAll.mean(0)[f], w=None)#1/np.sqrt(NAll.sum(0))[f])
...
#plt.savefig(outdir+'InfallMF.Aq.pdf')

beta=gamma/alpha
print alpha, beta, Mstar, Rstar
#final model
plt.figure()
plt.loglog(rSub[denSub>0], denSub[denSub>0], 'r')
