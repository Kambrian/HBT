...
frac,x=get_cum_distr(AqA1, fMinInfall, xbin, rmin, rmax)
plt.plot(xbin,frac, 'r-', label='A1')
frac,x=get_cum_distr(AqA2, fMinInfall, xbin, rmin, rmax)
plt.plot(xbin,frac, 'b-', label='A2')
#frac,x=get_cum_distr(HBTA2, fMinInfall, xbin, rmin, rmax)
#plt.plot(xbin,frac, 'c-', label='HBTA2')
fracAll=[]
xmin=[]
xall=[]
frac0All=[]
...
frac0All=np.array(frac0All)
print 'fs=', frac0All.mean(), '+-', frac0All.std()

y=np.histogram(x,xbin)
fracX=y[0][::-1].cumsum()*1./len(x)
fracX=np.hstack([0, fracX])[::-1]

#plt.plot(xbin,frac, 'g--')
#plt.plot(xbin[xbin>xmin], frac[xbin>xmin], 'g-', label='Aquarius')
plt.plot(xbin,fracX, 'k--')
plt.plot(xbin[xbin>xmin], fracX[xbin>xmin], 'k-', label='Aquarius')
for y in fracAll:
  plt.plot(xbin, y, ':')
plt.fill_between(xbin,frac+fracstd,frac-fracstd,color='g',alpha=0.3)
#plt.fill_between(xbin,fraclim[0],fraclim[1],color='g',alpha=0.3)
par=st.norm.fit(np.log(x[x>0]))
frac0=1.*np.sum(x>0)/len(x)
plt.plot(xbin, frac0*st.norm.sf(np.log(xbin),loc=par[0], scale=par[1]), '-', lw=3, alpha=0.5, color='k')
print par,frac0
#if H is PhA2:
  #par=st.norm.fit(np.log(x[x>0]))
  #frac0=1.*np.sum(x>0)/len(x)
...
#plt.savefig(outdir+'/StripCDF.Aquarius.pdf')
'''
##plot profiles
Rref=1
iInfall=0
