	...
	/*=========program IO params==========*/
	#define GASCAT_DIR  "/SANdisk5/kambrain/Sim6702Low/gascat"			//the output directory for gashalocat,gassubcat and gassrccat, this must be an existing directory
	#define SUBCAT_DIR  "/SANdisk5/kambrain/Sim6702Low/SubCat6"			//the output directory for subcatalogues and srccatalogues, this must be an existing directory
	#define GRPCAT_DIR  "/raid1/hywang/ReSim/SIM6702t/group_catalogue"				//the input directory for GrpCatalogues
	#define  SNAPSHOT_DIR  "/raid1/hywang/6test/6702g"					//the input directory for simulation snapshots
	#define LOGFILE_NAME  "logfile"																			//the name of program logfile, set to "stdout" to use stdout

	/*======simulation params===========*/
	#define NP_SIM	24558336 //total Number of all kinds of particles
	#define NP_gas	267396
	#define NP_DM	267396
	#define BOXSIZE	300000.0
	//#define partmass=0.158331,0.897207

	/*=======Tree algorithm params========*/
	#define TREE_ALLOC_FACTOR 3//  MaxNodes = ((maxnodes>500)?maxnodes:500);
	#define ErrTolTheta 0.45
	#define SofteningHalo 7.0
	#define NodeResolution 0.7  //0.1*Softening
	#define NodeReso_half 0.35
