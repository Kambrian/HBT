...
MASS_MAX_INFALL=min(MASS_MIN_INFALL*1e4, MASS_HOST/10.) #MASS_HOST/10. #
R_MIN=1e-3
R_MAX=2.
...
#np.save('min%.0e'%MASS_MIN_INFALL, np.array([x,y,x0,y0]))
...
plt.figure()
plt.loglog(data_old[2], data_old[3], 'bd', label='Duffy')
f=data_old[1]>0
par=np.polyfit(np.log(data_old[2][f]), np.log(data_old[3][f]), 1)
par[1]=np.exp(par[1])
x=np.array(plt.xlim())
plt.plot(x, par[1]*x**par[0], 'b-')

plt.loglog(data[2], data[3], 'gs', label='Prada')
f=data[1]>0
par=np.polyfit(np.log(data[2][f]), np.log(data[3][f]), 1)
par[1]=np.exp(par[1])
x=np.array(plt.xlim())
plt.plot(x, par[1]*x**par[0], 'g-')
