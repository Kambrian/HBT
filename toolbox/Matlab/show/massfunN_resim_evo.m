...
for i=1:N
    [Mlist,M0(i),R0(i)]=Msublist_in_radii(datadir,virtype,Nsnap(i),grpid,rmin,rmax,'bindata','norm');
...
yref=10^-3.02*(M0(end)*1e10)^0.1*(xref).^-1.9;
% yref=10^-1*(xref).^-1.9;
...
yref=10^-3.02*(M0(end)*1e10)^0.1*(xref).^-0.9;
plot(log10(xref),yref,'-k','displayname','G10,z=0');hold off;
set(gca,'yscale','log','yminortick','on');
xlabel('$\log(M_{sub}/M_{vir})$','interpreter','latex');
...
yref=1/0.9*10^-3.02*(M0(end)*1e10)^0.1*(xref).^-0.9;
plot(log10(xref),yref,'-k','displayname','G10,z=0');hold off;
set(gca,'yscale','log','yminortick','on');
xlabel('$\log(M_{sub}/M_{vir})$','interpreter','latex');
