...
disp('fitting gas...')
par_gas=gas_stripping_rate_stat(hist_bin,pmass_bin,Snappar,Chostpar,Csatpar);
filter=logical(par_gas(:,3)>0&Csatpar(:,1)>0&Csatpar(:,2)>0&Chostpar(:,1)>0&Chostpar(:,2)>0);
param_gas=[param_gas;par_gas(filter,:)];%alpha,beta,xi
snap_gas=[snap_gas;Snappar(filter,:)];
Mrate=[Mrate;Mratepar(filter,:)];
Mhost=[Mhost;Mhostpar(filter,:)*DMpmass(i)];
K=[K;Kpar(filter,:)];
j2=[j2;j2par(filter,:)];
Chost=[Chost;Chostpar(filter,:)];
Csat=[Csat;Csatpar(filter,:)];
hist_gas=[hist_gas;hist_bin(filter)];
pmass_gas=[pmass_gas;pmass_bin(filter)];
disp(['N=',num2str(numel(find(filter)))]);
...
par=param_gas(:,3);
figure;hist(par,20);xlabel('<\Delta f_{gas}>');ylabel('Counts');
x=linspace(0,0.4,40);
figure;h1=subplot(2,1,1);h2=subplot(2,1,2);
filter=logical(par<0.1);
y=histc(par(filter),x);
...
x=0.1:0.2:3;
figure;h1=subplot(2,1,1);h2=subplot(2,1,2);
filter=logical(param_gas(:,3)<0.1);
y=histc(param_gas(filter,1),x);
subplot(h1);plot(x+0.1,y,'k- .');xlabel('\alpha_{gas}');ylabel('Counts');
x=0.1:0.2:2;
y=histc(param_gas(filter,2),x);
subplot(h2);plot(x+0.1,y,'k- .');xlabel('\beta_{gas}');ylabel('Counts');

Msat=Mrate.*Mhost;MsatD=MrateD.*MhostD;
e2=1-j2;e2D=1-j2D;
disp('all done')
figure('name','parameter distributions');subplot(2,2,1);hist(param_gas(:,1));xlabel('\alpha_{gas}');subplot(2,2,2);hist(param_gas(:,2));xlabel('\beta_{gas}');
subplot(2,2,3);hist(param_dm(:,1));xlabel('\alpha_{dm}');subplot(2,2,4);hist(param_dm(:,2));xlabel('\beta_{dm}');

figure('name','Mass ratio dependence');subplot(2,2,1);loglog(Mrate(:,2),param_gas(:,1),'.');ylabel('\alpha_{gas}');xlabel('m/M');
subplot(2,2,2);loglog(Mrate(:,2),param_gas(:,2),'.');ylabel('\beta_{gas}');xlabel('m/M');
...
figure('name','Host Mass dependence');subplot(2,2,1);loglog(Mhost(:,2),param_gas(:,1),'.');ylabel('\alpha_{gas}');xlabel('M');
subplot(2,2,2);loglog(Mhost(:,2),param_gas(:,2),'.');ylabel('\beta_{gas}');xlabel('M');
...
figure('name','Sat Mass dependence');subplot(2,2,1);loglog(Msat(:,2),param_gas(:,1),'.');ylabel('\alpha_{gas}');xlabel('m');
subplot(2,2,2);loglog(Msat(:,2),param_gas(:,2),'.');ylabel('\beta_{gas}');xlabel('m');
...
figure('name','Host concentration dependence');subplot(2,2,1);loglog(Chost(:,2),param_gas(:,1),'.');ylabel('\alpha_{gas}');xlabel('Chost');
subplot(2,2,2);loglog(Chost(:,2),param_gas(:,2),'.');ylabel('\beta_{gas}');xlabel('Chost');
...
figure('name','Sat concentration dependence');subplot(2,2,1);loglog(Csat(:,1),param_gas(:,1),'.');ylabel('\alpha_{gas}');xlabel('Csat');
subplot(2,2,2);loglog(Csat(:,1),param_gas(:,2),'.');ylabel('\beta_{gas}');xlabel('Csat');
x=Csat(:,1);y=param_gas(:,2);x=x(y>0);y=y(y>0);p=polyfit(log(x),log(y),1);
...
figure('name','orbit shape dependence');subplot(2,2,1);loglog(e2(:,2),param_gas(:,1),'.');ylabel('\alpha_{gas}');xlabel('e2');
subplot(2,2,2);loglog(e2(:,2),param_gas(:,2),'.');ylabel('\beta_{gas}');xlabel('e2');
...
figure('name','orbital energy dependence');subplot(2,2,1);loglog(K(:,2),param_gas(:,1),'.');ylabel('\alpha_{gas}');xlabel('K');
subplot(2,2,2);loglog(K(:,2),param_gas(:,2),'.');ylabel('\beta_{gas}');xlabel('K');
...
figure;plot(param_both(:,2),param_both(:,5),'.');xlabel('\beta_{gas}');ylabel('\beta_{dm}');hold on;plot([0,3],[0,3]);
