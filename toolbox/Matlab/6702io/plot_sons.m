function h=plot_sons(subcat_in,subid,dims)
...
h=gca();
subind=subid+1;
plot(pos(subind,1),pos(subind,2),'rs');
hold on;
circleplt(pos(subind,1:2),rtidal(subind));
sonind=subcat.sub_hierarchy.sub(subind)+1;
if sonind>0
    plot([pos(subind,1),pos(sonind,1)],[pos(subind,2),pos(sonind,2)],'r-');
    plot(pos(sonind,1),pos(sonind,2),'r*');
    plotsub(sonind);
end
