...
#define KEV_U 3.48e-6  //E=KEV_U*U, transforms U into temperature (keV)
...
F.function=&rfun;
	...
	sprintf(buf,"%s/subprof_%03d.2",outputdir,Nsnap);myfopen(fpsubprof,buf,"w");
...
#define _CHECK if(dummy!=dummy2){printf("error!record brackets not match for Snap%d!\t%d,%d\n",Nsnap,dummy,dummy2);exit(1);} 	
	...
	_SKIP;	fread(&header,sizeof(header),1,fpsnap);	_SKIP2;	_CHECK;
	...
	printf("%f,%f,%f,%f,%f,%f,%d,%d\n",header.mass[0],header.mass[1],header.mass[2],header.mass[3],header.mass[4],header.mass[5],Nmass,Nother);
...
for(subid=SubCat.GrpOffset_Sub[grpid];subid<SubCat.GrpOffset_Sub[grpid+1];subid++)
{
	...
	rvir=Rtidal[subid];
	#else
	rvir=comoving_virial_radius(SubCat.SubLen[subid])*Factor_Relax;
	#endif
	for(i=0;i<3;i++)
	{
				...
				while(pid>=0)
				{
					...
					if(dr<rvir)
					{
						...
						else
						{
							...
							Lx=header.mass[0]*grho[pid]/(header.time*header.time*header.time)*sqrt(gU[pid])*integ*3040.7;
						}
						L_div[dr_bin]+=Lx;
	...
	for(i=0;i<NBIN;i++)
	{
		...
		fprintf(fpsubprof,"%g\t%d\t%d\n",rvir/Factor_Relax,SubCat.SubLen[subid],subid);
		//~ fflush(fpsubprof);
		//~ printf("%d\n",subid);
}
}
