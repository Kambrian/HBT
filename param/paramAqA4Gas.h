	...
	/*=========program IO params==========*/
	#define GASCAT_DIR  "/gpfs/data/jvbq85/HBT/data/AqA4Gas/gascat"			//the output directory for gashalocat,gassubcat and gassrccat, this must be an existing directory
	#define SUBCAT_DIR  "/gpfs/data/jvbq85/HBT/data/AqA4Gas/subcat"			//the output directory for subcatalogues and srccatalogues, this must be an existing directory
	//necessary sub-dirs: splitters,pro2dest,history
	#define GRPCAT_DIR  "/gpfs/data/arj/projects/jiaxin/run"				//the input directory for GrpCatalogues
	#define  SNAPSHOT_DIR  "/gpfs/data/arj/projects/jiaxin/run"					//the input directory for simulation snapshots
	#define LOGFILE_NAME  "logfile"																			//the name of program logfile, set to "stdout" to use stdout

	/*======simulation params===========*/
	#define NP_SIM	37706737L //total Number of all kinds of particles
	#define NP_gas 	18535972L
	#define NP_DM 	18535972L
	#define BOXSIZE 	100000.0  //kpc/h
	#define MP_DM   2.43771e-05
	#define MP_GAS   4.30184e-06  //10^10Msun/h; 
	
	/*=======Tree algorithm params========*/
	#define TREE_ALLOC_FACTOR 2//  MaxNodes = ((maxnodes>500)?maxnodes:500);
	#define ErrTolTheta 0.45
	#define SofteningHalo 0.25
	...
	#define MaxSnap 87//total number of snapshot outputs
	#define SNAPFILE_BASE "snap_run"
