...
for(Nsnap=0;Nsnap<MaxSnap;Nsnap++)
{
	...
	for(i=0;i<NList;i++)
	{
		...
		if((Nsnap>=SnapInfall)&&(OldNode->Mdm))
		{
			NNode[i]++;
...
{
	//subid_infall
	fwrite(&(ClusterHistory[HistList[i]].Member[ClusterHistory[HistList[i]].SnapEnter+1].SubID),sizeof(int),1,fp);
