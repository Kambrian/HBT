	...
	/*=========program IO params==========*/
	#define SUBCAT_DIR  "/gpfs/data/jvbq85/HBT/data/AqC2/subcat"			//the output directory for subcatalogues and srccatalogues, this must be an existing directory
	//necessary sub-dirs: splitters,pro2dest,history
	#define GRPCAT_DIR  "/gpfs/data/aquarius/halo_data/Aq-C/2"				//the input directory for GrpCatalogues
	#define  SNAPSHOT_DIR  "/gpfs/data/aquarius/halo_data/Aq-C/2"					//the input directory for simulation snapshots
	#define LOGFILE_NAME  "logfile"																			//the name of program logfile, set to "stdout" to use stdout

	/*======simulation params===========*/
	#define NP_SIM	691237649LL //total Number of all kinds of particles
	#define NP_GAS 	0
	#define NP_DM 	612602795LL
	#define BOXSIZE 100.0  //Mpc/h
	#define MP_DM  1.021477e-06 
	#define MP_GAS   0.
...
//	#define CONVERT_LENGTH_MPC_KPC //this tells to convert unit from Mpc to kpc, 
	...
	#define SNAPFILE_BASE "snap_C09_1200"
