...
//this is the default A2 subcat (renamed from A2more)
	/*=========program IO params==========*/
	#define SUBCAT_DIR  "/gpfs/data/jvbq85/HBT/data/AqA1/subcat"			//the output directory for subcatalogues and srccatalogues, this must be an existing directory
	//necessary sub-dirs: splitters,pro2dest,history
	#define GRPCAT_DIR  "/gpfs/data/aquarius/halo_data/Aq-A/1"				//the input directory for GrpCatalogues
	#define  SNAPSHOT_DIR  "/gpfs/data/aquarius/halo_data/Aq-A/1"					//the input directory for simulation snapshots
	#define LOGFILE_NAME  "logfile"																			//the name of program logfile, set to "stdout" to use stdout

	/*======simulation params===========*/
	#define NP_SIM	4397586154LL //total Number of all kinds of particles;
	#define NP_GAS 	0
	#define NP_DM 	4252607000LL//
	#define BOXSIZE 	100.0  //Mpc/h
	#define MP_DM    1.249995e-07
	#define MP_GAS   0.

	/*=======Tree algorithm params========*/
	#define TREE_ALLOC_FACTOR 2//  MaxNodes = ((maxnodes>500)?maxnodes:500);
	#define ErrTolTheta 0.45
	#define SofteningHalo 1.5e-5  //Mpc/h
	#define NodeResolution 1.5e-6  //0.1*Softening
	#define NodeReso_half 0.75e-6
...
//	#define CONVERT_LENGTH_MPC_KPC //this tells to convert unit from Mpc to kpc, 
	...
	#define SNAPFILE_BASE "snap_C02_2400_subidorder"
// 	#define SNAPLIST  11210     //snaplist identifier to enable the corresponding snaplist in iovars.c
	...
	#define NFILES_GRP 1000 //number of group files per snapshot
